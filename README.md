...
## Index Page
The home page has a couple of rendering issues. Firstly, when on mobile browsers the columns fill the horizontal width of the screen, rather than leaving some space either side. 

<details>
<summary>How it looks</summary>
<img src="docs/misrender.png">
</details>

<details>
<summary>How it should look</summary>
<img src="docs/correct-render.png">
</details>

Their is also a minor issue with the height of the content boxes not properly adjusting and instead matching the height of their content. 
...
This issue is also present on the club page, for the rules column. In both cases I believe the issue to be caused by the their being other layout tags wrapped inside the bootstrap tags. This makes some sense on the rules column, since it denotes the information as the information contained within, while related to the main content, is its own section. For the Homepage, this was to resolve an issue which I believe to be caused by how I'm nesting bootstrap containers, I beleive this is also the root of the issue with the width of the content on the home page when on mobile. 

##Site Wide
The entire site also suffered from a very slight horizontal scroll, generating a scroll bar when on mobile. While it didnt directly impact the UX, it was a less than positive experience causing the content to shift side by side at minor thumb strokes. This was resolved using overflow-x: hidden; in the CSS. I believe this may have been generated by how I am nesting bootstrap containers in the modal. 
